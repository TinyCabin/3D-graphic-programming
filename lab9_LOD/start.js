...
let center1 = glm.vec3(0,0,3);
let center2 = glm.vec3(0,0,3);
let center3 = glm.vec3(0,0,3);
let center4 = glm.vec3(0,0,3);
let center5 = glm.vec3(0,0,3);
...
async function loadFile(file) {
    ...
    for (let i=0; i<vertices.length-1; i++) {
        center_tmp.x += vertices[i][0];
        center_tmp.y += vertices[i][1];
        center_tmp.z += vertices[i][2];
    }
    center_tmp.x = center_tmp.x / (vertices.length-1);
    center_tmp.y = center_tmp.y / (vertices.length-1);
    center_tmp.z = center_tmp.z / (vertices.length-1);
    ...
    function renderScene() {
        ...
        if (!modelsLoaded) {
            ...
            if (buffer3 && points3) {
                ...
                sub.x = cameraPos.x - center3.x;
                sub.y = cameraPos.y - center3.y;
                sub.z = cameraPos.z - center3.z;
                distance3 = glm.length(sub);
                
                // LOD rendering based on distance
                if (distance3 >= 0 && distance3 < 5) {
                    // Level 0 - Highest detail (closest)
                    gl.drawArrays(gl.TRIANGLES, 0, 657408); 
                } else if (distance3 >= 5 && distance3 < 7) {
                    // Level 1 - High detail
                    ...
                    // Level 3 - Lowest detail (farthest)
